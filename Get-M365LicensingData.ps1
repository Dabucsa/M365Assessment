...
function Test-GraphContextRequirements {
    ...
    if ($RequiredTenantId -and $ContextTenantId -and $ContextTenantId -ne $RequiredTenantId) {
        return $false
    }
...
# Resumen de compra de seguridad para dashboard / renderer
...
$SecurityPurchaseItems = [System.Collections.Generic.List[object]]::new()
...
foreach ($Sku in $SkuSummary) {
    ...
    $IsPaidActive = ($Sku.Total -gt 0 -and -not ($Sku.FriendlyName -match '(?i)(free|trial|exploratory)'))
    if (-not $IsPaidActive -or $IncludedCategories.Count -eq 0) { continue }
    ...
    $SecurityPurchaseItems.Add([PSCustomObject]@{
...
$SecurityPurchaseItems = [System.Collections.Generic.List[object]]@(
    $SecurityPurchaseItems |
...
$SecurityPurchaseSummary = @{
    CoreE5Assigned            = $CoreE5Assigned
    CoreE3Assigned            = $CoreE3Assigned
    SecurityRelevantPaidSkus  = $SecurityPurchaseItems.Count
    TopSecurityPurchases      = @($SecurityPurchaseItems | Select-Object -First 6)
    Notes                     = @(
        "Core E5/E3 refleja bundles base Microsoft 365 / Office 365.",
        "SecurityRelevantPaidSkus cuenta SKUs de pago con categorias de seguridad detectadas; no equivale a seats unicos.",
        "TopSecurityPurchases ayuda a explicar suites y add-ons fuera del bundle base."
    )
}
...
$HasSignInActivity = -not $NoSignInActivity
$AllUsers = $null
...
if ($HasSignInActivity) {
    ...
    Write-Step "Large tenant mode: omitiendo SignInActivity para reducir tiempo y payload..."
}
...
# Exportar CSVs
$UserCsvPath = Join-Path $OutputPath "${OutputPrefix}_02_Users.csv"
$UserReport | Export-Csv -Path $UserCsvPath -NoTypeInformation -Encoding UTF8
Write-OK "Usuarios: $UserCsvPath"
if ($CompactUsersCsv) {
    Write-Warn "Users CSV generado en modo compacto (UPN/displayName) para reducir peso en tenants grandes"
}

if ($WasteReport.Count -gt 0) {
    $WasteCsvPath = Join-Path $OutputPath "${OutputPrefix}_04_Waste.csv"
...
if ($DuplicateReport.Count -gt 0) {
    $DupCsvPath = Join-Path $OutputPath "${OutputPrefix}_05_Duplicates.csv"
...
$AdoptionCsvPath = Join-Path $OutputPath "${OutputPrefix}_03_Adoption.csv"
...
$CapacityCsvPath = Join-Path $OutputPath "${OutputPrefix}_07_Capacity.csv"
...
$DeptCsvPath = Join-Path $OutputPath "${OutputPrefix}_06_Departments.csv"
...
$ReportData = @{
    ...
    SecurityPurchaseSummary  = $SecurityPurchaseSummary
...
Write-Host "    ${OutputPrefix}_02_Users.csv" -ForegroundColor White
Write-Host "    ${OutputPrefix}_03_Adoption.csv" -ForegroundColor White
if ($WasteReport.Count -gt 0)     { Write-Host "    ${OutputPrefix}_04_Waste.csv" -ForegroundColor White }
if ($DuplicateReport.Count -gt 0) { Write-Host "    ${OutputPrefix}_05_Duplicates.csv" -ForegroundColor White }
...
if ($ManagedSession) {
    ...
    Write-OK "Sesion existente mantenida`n"
}
