...
function Test-GraphContextRequirements {
    ...
    $tenantMatches = (-not $ExpectedTenantId) -or (-not $tenantId) -or ($tenantId -eq $ExpectedTenantId)
...
function Get-LicensedUserCountForProducts {
    param(
    ...
    $Matches = @($AdoptionRows | Where-Object { $Products -contains $_.Product })
    if (-not $Matches -or $Matches.Count -eq 0) {
        return 0
    }

    $Best = $Matches | Sort-Object @{ Expression = { [int]($_.UsersEnabled + $_.UsersDisabled) }; Descending = $true } | Select-Object -First 1
    if (-not $Best) {
        return 0
    }

    return [int]($Best.UsersEnabled + $Best.UsersDisabled)
}
...
if (-not $JsonFile) {
    ...
    Write-Warn "No se encontro report_data.json en '$Hint'. Ejecuta primero Get-MSLicensingReport.ps1"
    exit 1
}
...
$UserCsvFile = if ($UsersCsvPath) {
    if (Test-Path -LiteralPath $UsersCsvPath) { Get-Item -LiteralPath $UsersCsvPath }
    else { $null }
} else {
    Get-ChildItem -Path $OutputPath -Filter "*_02_Users.csv" -ErrorAction SilentlyContinue |
        Sort-Object LastWriteTime -Descending | Select-Object -First 1
}
...
try {
    ...
    if ($ExistingCtxCheck.isUsable) {
        $Context = $ExistingCtx
        Write-OK "Reusando sesion existente: $($ExistingCtxCheck.account)"
    } else {
        ...
        Write-OK "Conectado en sesion de proceso como: $($CurrentCtxCheck.account)"
    }
    Set-ModuleStatus -Name "GraphConnection" -Status "success" -Message "Sesion Graph validada para tenant y scopes requeridos." -Details @{
        account = (Test-GraphContextRequirements -Context $Context -ExpectedTenantId $ExpectedTenantId -RequiredScopes $Scopes).account
        tenantId = (Get-ContextTenantId -Context $Context)
    ...
    try {
        ...
        if ($CaGraphFailures.Count -gt 0) {
            ...
            Set-ModuleStatus -Name "ConditionalAccess" -Status "success" -Message "Conditional Access leido correctamente." -Details @{
                ...
                disabled = $CaDisabled.Count
                hasAllUsersPolicy = $AllUsersPolicy
                licensedCoveragePct = $LicCoveragePct
            }
        }
    } catch {
    ...
    try {
        ...
        if ($LicCount -gt 0 -and $LicCount -le 500) {
            ...
            if ($MfaGraphFailures.Count -gt 0) {
                Set-ModuleStatus -Name "MFA_SSPR" -Status "warning" -Message "MFA y SSPR se calcularon con advertencias de Graph." -Details @{
                    totalUsers = $TotalLic
                    mfaPct = $MfaPct
                    ssprPct = $SsprPct
                ...
                Set-ModuleStatus -Name "MFA_SSPR" -Status "success" -Message "MFA y SSPR leidos correctamente para usuarios licenciados." -Details @{
                    totalUsers = $TotalLic
                    mfaPct = $MfaPct
                    ssprPct = $SsprPct
                }
...
if ($HasEntraP2) {
    ...
    if ($HasEntraGov) {
        ...
        Set-ModuleStatus -Name "AccessReviews" -Status "skipped" -Message "Modulo omitido: Entra Governance no detectado en licenciamiento."
    }
} else {
    Set-ModuleStatus -Name "RiskyUsers" -Status "skipped" -Message "Modulo omitido: Entra ID P2 no detectado en licenciamiento."
    Set-ModuleStatus -Name "PIM" -Status "skipped" -Message "Modulo omitido: Entra ID P2 no detectado en licenciamiento."
    Set-ModuleStatus -Name "AccessReviews" -Status "skipped" -Message "Modulo omitido: Entra ID P2 no detectado en licenciamiento."
...
"@
            ...
            $MdeLicensedUsers = 0
            if ($LicData.Adoption) {
                $MdeLicensedUsers = Get-LicensedUserCountForProducts -AdoptionRows $LicData.Adoption -Products @("MDE_P2", "MDE_P1")
            }
...
"@
            ...
            Write-OK "MDA: $(if($MdaStats -and $MdaStats[0]){$MdaStats[0].UniqueApps}else{0}) apps monitoreadas, $(if($MdaStats -and $MdaStats[0]){$MdaStats[0].TotalEvents}else{0}) eventos en $HuntingWindowLabel"
...
"@
            ...
            Write-OK "MDI: $($DCList.Count) DCs monitoreados, $(if($MdiStats -and $MdiStats[0]){$MdiStats[0].TotalEvents}else{0}) logon events en $HuntingWindowLabel"
            ...
            if (-not $Result.ContainsKey($ProductKey)) {
                if ($ProductKey -eq "MDO") {
                    ...
                    $Result.MDE = @{
                        ...
                        UsersWithLicense = $MdeLicensedUsers; DevicesStale7d = 0
                        Alerts30d = @{ Total = 0; High = 0; Medium = 0 }
                        Note = "Datos KQL no disponibles. Configuracion evaluada via Secure Score."
                    }
...
# --- Entra ID P1: Conditional Access, MFA y SSPR ---
if ($Result.ContainsKey('Entra') -and $Result.Entra) {
    $Ca = $Result.Entra.ConditionalAccess
    $Mfa = $Result.Entra.MFA
    $Sspr = $Result.Entra.SSPR
    if ($Ca -or $Mfa -or $Sspr) {
        $CaEnabled = if ($Ca) { [int]$Ca.Enabled } else { 0 }
        $HasAllUsersPolicy = if ($Ca) { [bool]$Ca.HasAllUsersPolicy } else { $false }
        $MfaRegistered = if ($Mfa) { [int]$Mfa.Registered } else { 0 }
        $SsprRegistered = if ($Sspr) { [int]$Sspr.Registered } else { 0 }
        $TW["Entra_ID_P1"] = @{
            Active = [bool](($CaEnabled -gt 0) -or ($MfaRegistered -gt 0) -or ($SsprRegistered -gt 0))
            Policies = @(
                @{ Name = "Conditional Access habilitado"; Active = [bool]($CaEnabled -gt 0) }
                @{ Name = "CA para todos los usuarios"; Active = $HasAllUsersPolicy }
                @{ Name = "MFA registrado"; Active = [bool]($MfaRegistered -gt 0) }
                @{ Name = "SSPR registrado"; Active = [bool]($SsprRegistered -gt 0) }
            )
            ConditionalAccessEnabled = $CaEnabled
            HasAllUsersPolicy = $HasAllUsersPolicy
            MfaRegistered = $MfaRegistered
...
if ($Result.ContainsKey('MDO') -and $Result.MDO) {
    Write-Host "    MDO Emails ${HuntingWindowLabel}:   $($Result.MDO.EmailsProcessed30d) (Phishing:$($Result.MDO.PhishingDetected))" -ForegroundColor Green
}
if ($Result.ContainsKey('MDA') -and $Result.MDA) {
