...
$RunId = if ($RunId) { $RunId } else { Get-Date -Format "yyyyMMdd_HHmm" }
...
if ($RunLic) {
    ...
    try {
        ...
        $ExistLic   = Get-Item (Join-Path $OutputPath "${RunId}_report_data.json") -ErrorAction SilentlyContinue
        $ExistUsers = Get-Item (Join-Path $OutputPath "${RunId}_02_Users.csv") -ErrorAction SilentlyContinue
        if (-not $ExistLic)   { $ExistLic   = Find-Latest $OutputPath "*_report_data.json" }
        if (-not $ExistUsers) { $ExistUsers = Find-Latest $OutputPath "*_02_Users.csv" }
    } catch {
        $Results.Lic = "ERROR: $_"
...
if ($RunScore) {
    ...
    try {
        ...
        if (-not $ExistScore) { $ExistScore = Find-Latest $OutputPath "*_secure_score.json" }
    } catch {
...
if ($RunAdopt) {
    ...
    try {
        ...
        if ($ExistUsers) { $AdoptParams.UsersCsvPath = $ExistUsers.FullName }
        ...
        if (-not $ExistAdopt) { $ExistAdopt = Find-Latest $OutputPath "*_security_adoption.json" }
    } catch {
